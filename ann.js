...
async function main(){
  try {
    ...
    for (const cfg of grid) {
      ...
      try {
        ...
        const predsScaled = X_test.map(r => {
          ...
          const real = out * (yMax - yMin) + yMin;
          return real;
        });

        const mae = meanAbs(Y_test, predsScaled);
        const rmse = rmseCalc(Y_test, predsScaled);
        const within20 = percentWithin(Y_test, predsScaled, 20);
        console.log(` -> MAE ${mae.toFixed(2)}, RMSE ${rmse.toFixed(2)}, %within±20 ${within20.toFixed(1)}%`);

        if (mae < best.mae) {
          best.mae = mae;
          best.params = cfg;
          best.netJSON = net.toJSON();
          best.preds = predsScaled;
        }
      } catch (err) {
    ...
    if (!best.netJSON) { console.error('No ANN model trained successfully'); process.exit(1); }

    // Save ANN JSON and also write aft_model.json envelope so app can detect ANN active model
    fs.writeFileSync('best_model_ann.json', JSON.stringify(best.netJSON, null, 2));
